...
def _side_words(store: Store, org_id: int) -> dict[str, str]:
    ...
    return {"noun": words.noun, "label": words.label,
            "weaker": words.weaker, "strong": words.strong,
            "both": words.both, "weaker_es": words.weaker_es}
...
def log_training(
    ...
    pays. Inflate it and the app tells you to rest. Hide it and the app stays
    quiet when it should not have.
    """
    from datetime import date as _date
    try:
        day = _date.fromisoformat(body.day) if body.day else None
    except ValueError:
