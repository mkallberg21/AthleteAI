...
def for_athlete(store, athlete_id: int) -> WeekPlan:
    ...
    # assignments.for_athlete already returns dicts carrying their own
    # progress and days_remaining. This used to re-wrap them as if they were
    # Assignment objects, which raised AttributeError the moment an athlete
    # actually had an open assignment -- so the week-plan card 500'd for
    # exactly the athletes it was written for, and worked for nobody else
    # because there was nothing to show. Take what that function returns.
    result_assignments = assignments_for(store.conn, athlete_id, only_open=True)
    ...
    # _compose_line reads "band", "status" and "minutes", which live on the
    # report's budget section rather than at its top level. Passing the whole
    # report made every one of those lookups miss its default, so the sentence
    # told a thirteen-year-old to aim for "about 0 minutes across 0 days".
    # The payload still carries the full report; only the line reads the part
    # it was written for.
    line = _compose_line(
        result_assignments, budget_report.get("budget") or {}, film_today, age
    )
    return WeekPlan(
...
def _compose_line(
    ...
    if not assignments and budget_status == Status.UNKNOWN and not film_clips_left:
        if is_young:
            return "Nothing set for this week yet, so have fun with the ball when you feel like it."
    ...
    if budget_status == Status.UNKNOWN:
        if is_young:
            ...
            parts.append(
                ...
                f"this week, a few short sessions rather than an evening job."
            )
